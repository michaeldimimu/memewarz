...
const MemeGenerating = ({
  ...
  players: { name: string; hasSubmitted?: boolean }[];
}) => {
  return (
    <div className="flex flex-col items-center w-full max-w-sm mx-auto px-4 gap-8">
      {players.map((player) => (
        <div key={player.name} className="w-full">
          ...
          {/* Loading / Ready card */}
          <div className="w-full aspect-square rounded-2xl bg-neutral-light-200 flex flex-col items-center justify-center gap-4 border border-gray-200">
            {player.hasSubmitted ? (
              <>
                <p className="text-xl font-semibold text-neutral-dark-200">
                  Meme Ready!
                </p>
                <CheckCircle2
